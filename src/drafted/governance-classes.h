 ...
 *
 *   // network
 *   CGoaCoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.goacoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CGoaCoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CGoaCoinNetwork(UniValue objIn)
    {
...
// // can be under: GoaCoinNetwork
// //   -- signature requirements : Key1(User)
// class CGoaCoinNetworkVariable : public CGovernanceObject
// {
// private:
