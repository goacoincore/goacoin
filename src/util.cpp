...
const char * const BITCOIN_CONF_FILENAME = "goacoin.conf";
const char * const BITCOIN_PID_FILENAME = "goacoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "goacoin" is a composite category enabling all GoaCoin-related debug output
            if(ptrCategory->count(string("goacoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GoaCoin Core
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GoaCoin Core
    // Mac: ~/Library/Application Support/GoaCoin Core
    // Unix: ~/.goacoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GoaCoin Core";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GoaCoin Core";
#else
    // Unix
    return pathRet / ".goacoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty goacoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override goacoin.conf
