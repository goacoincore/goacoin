...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"goacoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given goacoinaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"togoacoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Wallet encrypted; GoaCoin Core server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
