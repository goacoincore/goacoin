...
*Setup instructions for a Gitian build of GoaCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GoaCoin Core executables. It provides a way to be reasonably sure that the
...
- [Building GoaCoin Core](#building-goacoin-core)
...
Clone the git repositories for GoaCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/goacoincore/goacoin
```
...
in the GoaCoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build GoaCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the GoaCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/goacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/goacoincore/goacoin
...
For example:
```bash
URL=https://github.com/crowning-/goacoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit goacoin=${COMMIT} --url goacoin=${URL} ../goacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit goacoin=${COMMIT} --url goacoin=${URL} ../goacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit goacoin=${COMMIT} --url goacoin=${URL} ../goacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the goacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../goacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/goacoin.git
SIGPATH=/some/root/path/goacoin-detached-sigs.git

./bin/gbuild --url goacoin=${BTCPATH},signature=${SIGPATH} ../goacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/goacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/goacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/goacoin-osx-build.assert
```
...
[goacoin/gitian.sigs](https://github.com/goacoincore/gitian.sigs/) repository, or if that's not possible to create a pull
request.
